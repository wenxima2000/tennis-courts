---
name: tennis-courts
description: 查看悉尼网球场空闲时段（Centennial Park / Moore Park / Jensens 各场地，intrac 预订系统）
...
```bash
node check-courts.mjs [--date YYYY-MM-DD] [--site parklands|jensens|all] [--location <名称>] [--days N] [--headless]
```

参数说明：
- `--date`：悉尼当地日期，默认今天。用户说"明天"就换算成对应日期。
- `--site`：`parklands`（Centennial Park / Moore Park）、`jensens`（Surry Hills、Alexandria、Beaconsfield、Glebe、Rosebery），默认 `all`。
- `--location`：按名称模糊过滤场地，如 `--location alexandria`。
- `--days`：连续查几天，默认 1。
- `--headless`：无窗口运行。Parklands 总是可以无头；Jensens 需要登录，只有在会话还有效时才能无头。

行为要点：
- 两个站都在 AWS WAF 后面，必须用脚本里的 Playwright 浏览器，不要用 curl/FetchURL。
